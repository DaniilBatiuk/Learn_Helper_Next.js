...
type ITranslationForm = {
  setTranslation: React.Dispatch<React.SetStateAction<TranslationResult>>;
  translation: TranslationResult;
};

export function TranslationForm({ setTranslation, translation }: ITranslationForm) {
  ...
  const onSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    const languageFrom = formData.get("languageFrom")!.toString();
    const languageTo = formData.get("languageTo")!.toString();
    const res = await TranslationService.getTranslation(inputRef.current!.value.toString(), languageFrom, languageTo);
    setTranslation(() => res);

    if (inputRef.current && inputRef.current?.value !== "") {
      const isExist = await DictionaryService.isExistInDictionary(inputRef.current.value);
      setIsExist(isExist);
    }
  };
        ...
        <div className="form__icon__input">
          ...
          <svg
            ...
            onClick={() => {
              if (inputRef.current && inputRef.current?.value !== "") {
                setIsExist(true);
                const uniqueArray = translation.translations.reduce((accumulator: string[], currentValue: string) => {
                  if (!accumulator.includes(currentValue)) {
                    accumulator.push(currentValue);
                  }
                  return accumulator;
                }, []);
                DictionaryService.addDictionary({ word: inputRef.current.value.toLowerCase(), translations: uniqueArray.slice(0, 3) });
              }
            }}
          >
            <path
